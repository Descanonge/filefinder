...
def test_group_str(ref: GroupSpecs, idx: int):
...
def test_definition_parsing(ref: GroupSpecs):
...
def test_fix_format_number(ref: FormatSpecs, number):
...
def test_fix_value_string(ref: FormatSpecs, value):
